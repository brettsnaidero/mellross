...
// Home slideshow
var wallopEl = document.querySelectorAll('.Wallop');
Array.from(wallopEl).forEach(function(slider) {
    var slideTime = new Wallop(slider);

		// Caption
		// var captionElement = slideTime.$selector.getAttribute('data-captions');
    // captionElement = document.getElementById(captionElement); // Update caption
    // slideTime.on('change', function(event) {
    //     var num = event.detail.currentItemIndex;
    //     var caption = slideTime.$selector.children[0].children[num].children[0].children[0].cloneNode(true); // captionElement.classList.remove('on');
    //     captionElement.innerHTML = '';
    //     captionElement.appendChild(caption); // captionElement.classList.add('on');
    // });
});
